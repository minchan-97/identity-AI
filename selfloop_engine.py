...
class SelfLoopState:
    ...
    def save(self, path):
        ...
        # 정체성 기억 저장 (LLM 호출을 넘어 지속되는 사고 구조)
        if getattr(self, "identity", None) is not None:
            from dataclasses import asdict as _asdict
            blob["identity"] = _asdict(self.identity)
    ...
    def load(cls, path):
        ...
        if "baseline" in b:
            try:
                ...
                pass
        # 정체성 기억 복원 (재호출 시 정체성 지속)
        if "identity" in b:
            try:
                from identity_core import IdentityMemory
                st.identity = IdentityMemory(**b["identity"])
            except Exception:
                pass
        return st
