...
with page[1]:
    ...
    if not stt.corpus:
        ...
        # ── 정체성 편집 (pkl에 지속, 매 답변에 강제 주입) ──
        if getattr(stt, "identity", None) is not None:
            idn = stt.identity
            with st.expander("🧬 정체성 (매 답변에 강제 주입 · pkl에 지속)", expanded=False):
                st.caption("여기 설정한 정체성이 LLM 호출마다 system_prompt로 주입됩니다. "
                           "LLM이 바뀌어도 pkl을 이어 쓰면 같은 정체성으로 답합니다.")
                idn.persona = st.text_area("페르소나 (성격·역할·말투)",
                                           value=idn.persona, height=70)
                vtxt = st.text_area("가치관 (한 줄에 하나)",
                                    value="\n".join(idn.values), height=70)
                rtxt = st.text_area("판단 기준 (한 줄에 하나)",
                                    value="\n".join(idn.judgment_rules), height=70)
                if st.button("정체성 갱신"):
                    idn.values = [v.strip() for v in vtxt.split("\n") if v.strip()]
                    idn.judgment_rules = [r.strip() for r in rtxt.split("\n") if r.strip()]
                    st.success("정체성 갱신됨. pkl 저장 시 지속됩니다.")
                # 정체성 현황
                st.caption(f"축적 지식 {len(idn.learned_facts)}개 · "
                           f"대화 기억 {len(idn.episodic)}개 · "
                           f"수정 이력 {len(idn.revisions)}건")
        ...
        if st.button("▶ 답변 생성", type="primary") and q.strip():
            ...
            with cL:
                ...
                # 정체성 강제 주입: pkl에 지속된 정체성을 system_prompt로
                identity_sys = None
                if getattr(stt, "identity", None) is not None:
                    identity_sys = stt.identity.to_system_prompt(
                        base="당신은 주어진 문서 맥락에 근거해 답하는 조력자입니다.")
                ...
                if save_qa and ans and not ans.startswith("["):
                    ...
                    # 정체성에도 흡수 (자기진화 — 강제 기억 주입)
                    if getattr(stt, "identity", None) is not None:
                        stt.identity.absorb(q, ans)
                    st.info("질문·답변을 코퍼스와 정체성 기억에 누적했습니다. "
                            "pkl 저장 시 다음 호출에도 이 정체성이 이어집니다.")
